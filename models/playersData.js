let teamLogos = ["https://upload.wikimedia.org/wikipedia/en/thumb/0/01/KK_Crvena_zvezda_logo.svg/800px-KK_Crvena_zvezda_logo.svg.png",
                 ...
                 'https://upload.wikimedia.org/wikipedia/it/a/a3/Logo_Valencia_Basket_Club.png',
                 'https://upload.wikimedia.org/wikipedia/en/thumb/6/6b/Maccabi_Tel_Aviv_BC_logo.svg/1200px-Maccabi_Tel_Aviv_BC_logo.svg.png',
                 'https://upload.wikimedia.org/wikipedia/en/thumb/3/3d/Pallacanestro_Olimpia_Milano_logo.svg/800px-Pallacanestro_Olimpia_Milano_logo.svg.png',
...
// ----------------------------------------NE PRILAZI GUFANE -----------------------------------------------------------
    ...
    let nikolaKalinic = new Player('Nikola Kalinic', [teamLogos[3], 'FCB', 'CZV' , 'VAL', 'FEN'], ['F'], '201', '32', '3');  playersData.push(nikolaKalinic)
    ...
    let filipPetrusev = new Player('Filip Petrusev', [teamLogos[7], 'OLY' , 'EFE', 'ZVE' ], ['F', 'C'], '211', '23', '30');  playersData.push(filipPetrusev)
    ...
    let nateReuvers = new Player('Nate Reuvers', [teamLogos[10], 'VAL' ], ['C'], '211', '25', '3');  playersData.push(nateReuvers)
    let jaimePradilla = new Player('Jaime Pradila', [teamLogos[10], 'VAL' ], ['F','C'], '205', '23', '4');  playersData.push(jaimePradilla)
    let justinAnderson = new Player('Justin Anderson', [teamLogos[10], 'VAL' ], ['G','F'], '196', '30', '5');  playersData.push(justinAnderson)
    let xabierLopez_Arostegui = new Player('Xabier Lopez-Arostegui', [teamLogos[10], 'VAL' ], ['F'], '200', '26', '6');  playersData.push(xabierLopez_Arostegui)
    let chrisJones = new Player('Chris Jones', [teamLogos[10], 'VAL','ASV','MAC' ], ['G'], '188', '30', '7');  playersData.push(chrisJones)
    let damienInglis = new Player('Damien Inglis', [teamLogos[10], 'VAL','ASM' ], ['F'], '203', '28', '10');  playersData.push(damienInglis)
    let stefanJovic = new Player('Stefan Jovic', [teamLogos[10], 'VAL','CZV','PAO','BAY' ], ['G'], '198', '33', '16');  playersData.push(stefanJovic)
    let boubacarToure = new Player('Boubacar Toure', [teamLogos[10], 'VAL' ], ['C'], '213', '28', '21');  playersData.push(boubacarToure)
    let kassiusRobertson = new Player('Kassius Robertson', [teamLogos[10], 'VAL' ], ['G'], '191', '29', '30');  playersData.push(kassiusRobertson)
    let brandonDavies = new Player('Brandon Davies', [teamLogos[10], 'VAL','EA7','FCB','ZAL','ASM' ], ['C'], '208', '32', '32');  playersData.push(brandonDavies)
    let semiOjeleye = new Player('Semi Ojeleye', [teamLogos[10], 'VAL','VIR' ], ['F'], '198', '29', '37');  playersData.push(semiOjeleye)
    let kevinPangos = new Player('Kevin Pangos', [teamLogos[10], 'VAL','EA7','FCB','ZAL' ], ['G'], '185', '31', '47');  playersData.push(kevinPangos)

    let antoniusCleveland = new Player('Antonius Cleveland', [teamLogos[11], 'MAC' ], ['G','F'], '196', '30', '1');  playersData.push(antoniusCleveland)
    let jamesWebb = new Player('James Webb', [teamLogos[11], 'MAC','VAL' ], ['F'], '206', '30', '3');  playersData.push(jamesWebb)
    let lorenzoBrown = new Player('Lorenzo Brown', [teamLogos[11], 'MAC','CZV','FEN' ], ['G'], '196', '33', '4');  playersData.push(lorenzoBrown)
    let wadeBaldwin = new Player('Wade Baldwin', [teamLogos[11], 'MAC','OLY','BSK','BAY' ], ['G'], '193', '27', '5');  playersData.push(wadeBaldwin)
    let rafiMenco = new Player('Rafi Menco', [teamLogos[11], 'MAC' ], ['F'], '200', '29', '8');  playersData.push(rafiMenco)
    let romanSorkin = new Player('Roman Sorkin', [teamLogos[11], 'MAC' ], ['F','C'], '208', '27', '9');  playersData.push(romanSorkin)
    let johnDiBartolomeo = new Player('John DiBartolomeo', [teamLogos[11], 'MAC' ], ['G'], '183', '32', '12');  playersData.push(johnDiBartolomeo)
    let jasielRivero = new Player('Jasiel Rivero', [teamLogos[11], 'MAC','VAL' ], ['F','C'], '206', '30', '14');  playersData.push(jasielRivero)
    let jakeCohen = new Player('Jake Cohen', [teamLogos[11], 'MAC' ], ['F','C'], '209', '33', '15');  playersData.push(jakeCohen)
    let joeThomasson = new Player('Joe Thomasson', [teamLogos[11], 'MAC' ], ['G'], '193', '30', '24');  playersData.push(joeThomasson)
    let joshNebo = new Player('Josh Nebo', [teamLogos[11], 'MAC' ], ['C'], '206', '26', '32');  playersData.push(joshNebo)
    let tamirBlatt = new Player('Tamir Blatt', [teamLogos[11], 'MAC','ALB' ], ['G'], '178', '26', '45');  playersData.push(tamirBlatt)
    let bonzieColson = new Player('Bonzie Colson', [teamLogos[11], 'MAC' ], ['F'], '198', '28', '50');  playersData.push(bonzieColson)

    let rodneyMcGruder = new Player('Rodney McGruder', [teamLogos[12], 'EA7' ], ['G'], '193', '28', '1');  playersData.push(rodneyMcGruder)
    let maodoLo = new Player('Maodo Lo', [teamLogos[12], 'EA7','ALB','BAY' ], ['G'], '191', '31', '0');  playersData.push(maodoLo)
    let alexPoythress = new Player('Alex Poythress', [teamLogos[12], 'EA7','MAC' ], ['F','C'], '206', '30', '2');  playersData.push(alexPoythress)
    let giordanoBortolani = new Player('Giordano Bortolani', [teamLogos[12], 'EA7' ], ['G'], '193', '23', '3');  playersData.push(giordanoBortolani)
    let stefanoTonut = new Player('Stefano Tonut', [teamLogos[12], 'EA7' ], ['G'], '194', '30', '7');  playersData.push(stefanoTonut)
    let nicoloMelli = new Player('Nicolo Melli', [teamLogos[12], 'EA7','FEN' ], ['F','C'], '205', '33', '9');  playersData.push(nicoloMelli)
    let billyBaron = new Player('Billy Baron', [teamLogos[12], 'EA7','CZV' ], ['G'], '188', '33', '12');  playersData.push(billyBaron)
    let shabazzNapier = new Player('Shabazz Napier', [teamLogos[12], 'EA7','CZV' ], ['G'], '183', '32', '13');  playersData.push(shabazzNapier)
    let giampaoloRicci = new Player('Giampaollo Ricci', [teamLogos[12], 'EA7','VIR' ], ['F'], '202', '32', '17');  playersData.push(giampaoloRicci)
    let diegoFlaccadori = new Player('Diego Flaccadori', [teamLogos[12], 'EA7','BAY' ], ['G'], '195', '27', '21');  playersData.push(diegoFlaccadori)
    let devonHall = new Player('Devon Hall', [teamLogos[12], 'EA7' ], ['G'], '196', '28', '22');  playersData.push(devonHall)
    let shavonShields = new Player('Shavon Shields', [teamLogos[12], 'EA7','BSK' ], ['F'], '201', '29', '31');  playersData.push(shavonShields)
    let nikolaMirotic = new Player('Nikola Mirotic', [teamLogos[12], 'EA7','FCB','RMB' ], ['F'], '208', '33', '33');  playersData.push(nikolaMirotic)
    let kyleHines = new Player('Kyle Hines', [teamLogos[12], 'EA7','OLY' ], ['F','C'], '198', '37', '42');  playersData.push(kyleHines)
    let johannesVoigtmann = new Player('Johannes Voigtman', [teamLogos[12], 'EA7','BSK' ], ['F','C'], '211', '31', '77');  playersData.push(johannesVoigtmann)
    
    let keenanEvans = new Player('Keenan Evans',[teamLogos[13],'ZAL','MAC'],['G'], '191','27','2'); playersData.push(keenanEvans)
    let lukasLekavicius = new Player('Lukas Lekavicius',[teamLogos[13],'ZAL','PAO'],['G'], '180','29','4'); playersData.push(lukasLekavicius)
    let kevarriusHayes = new Player('Kevarius Hayes',[teamLogos[13],'ZAL','ASV'],['C'], '206','26','8'); playersData.push(kevarriusHayes)
    let dovydasGiedraitis = new Player('Dovydas Giedraitis',[teamLogos[13],'ZAL'],['G'], '193','23','9'); playersData.push(dovydasGiedraitis)
    let rolandsSmits = new Player('Rolands Smits',[teamLogos[13],'ZAL','FCB'],['F','C'], '208','28','10'); playersData.push(rolandsSmits)
    let laurynasBirutis = new Player('Laurynas Birutis',[teamLogos[13],'ZAL'],['C'], '213','26','15'); playersData.push(laurynasBirutis)
    let karolisLukosiunas = new Player('Karolis Lukosiunas',[teamLogos[13],'ZAL'],['G'], '195','26','16'); playersData.push(karolisLukosiunas)
    let edmondSumner = new Player('Edmond Sumner',[teamLogos[13],'ZAL'],['G'], '197','28','26'); playersData.push(edmondSumner)
    let tomasDimsa = new Player('Tomas Dimsa',[teamLogos[13],'ZAL'],['G'], '196','30','33'); playersData.push(tomasDimsa)
    let bradyManek = new Player('Brady Manek',[teamLogos[13],'ZAL'],['F'], '206','25','45'); playersData.push(bradyManek)
    let arnasButkevicius = new Player('Arnas Butkevicius',[teamLogos[13],'ZAL'],['F'], '197','31','51'); playersData.push(arnasButkevicius)
    let edgarasUlanovas = new Player('Edgaras Ulanovas',[teamLogos[13],'ZAL','FEN'],['F'], '199','32','92'); playersData.push(edgarasUlanovas)
    
    let shaneLarkin = new Player('Shane Larkin',[teamLogos[14],'EFE','BSK'],['G'], '183','31','0'); playersData.push(shaneLarkin)
    let rodrigueBeaubois = new Player('Rodrigue Beaubois',[teamLogos[14],'EFE','BSK'],['G'], '188','35','1'); playersData.push(rodrigueBeaubois)
    let elijahBryant = new Player('Elijah Bryant',[teamLogos[14],'EFE','MAC'],['G','F'], '196','28','6'); playersData.push(elijahBryant)
    let ridvanOncel = new Player('Ridvan Oncel',[teamLogos[14],'EFE'],['G'], '191','26','10'); playersData.push(ridvanOncel)
    let ertenGazi = new Player('Erten Gazi',[teamLogos[14],'EFE'],['G'], '191','26','11'); playersData.push(ertenGazi)
    let willClyburn = new Player('Will Clyburn',[teamLogos[14],'EFE'],['F'], '201','33','12'); playersData.push(willClyburn)
    let dariusThompson = new Player('Darius Thompson',[teamLogos[14],'EFE','BSK'],['G'], '193','28','13'); playersData.push(dariusThompson)
    let dogusOzdemiroglu = new Player('Dogus Ozdemiroglu ',[teamLogos[14],'EFE'],['G'], '191','27','18'); playersData.push(dogusOzdemiroglu)
    let yildizliBurakCan = new Player('Yildizli Burak Can',[teamLogos[14],'EFE'],['F'], '203','29','19'); playersData.push(yildizliBurakCan)
    let tiborPleiss = new Player('Tibor Pleiss',[teamLogos[14],'EFE','BSK','VAL','FCB'],['C'], '221','34','21'); playersData.push(tiborPleiss)
    let justusHollatz = new Player('Justus Hollatz',[teamLogos[14],'EFE'],['G'], '191','22','23'); playersData.push(justusHollatz)
    let osmaniErcan = new Player('Osmani Ercan',[teamLogos[14],'EFE'],['F','C'], '206','25','24'); playersData.push(osmaniErcan)
    let danielOturu = new Player('Daniel Oturu',[teamLogos[14],'EFE'],['C'], '208','24','25'); playersData.push(danielOturu)
    let mikeDaum = new Player('Miek Daum',[teamLogos[14],'EFE'],['F'], '206','28','31'); playersData.push(mikeDaum)
    let erkanYilmaz = new Player('Erkan Yilmaz',[teamLogos[14],'EFE'],['F'], '196','26','33'); playersData.push(erkanYilmaz)
    let derekWillis = new Player('Derek Willis',[teamLogos[14],'EFE'],['F'], '206','28','35'); playersData.push(derekWillis)
    let TyriqueJones = new Player('Tyrique Jones',[teamLogos[14],'EFE'],['F','C'], '206','26','88'); playersData.push(TyriqueJones)
    
    let nickWeiler_Babb = new Player('Nick Weiller-Babb',[teamLogos[15],'BAY'],['G'], '196','28','0'); playersData.push(nickWeiler_Babb)
    let sylvianFrancisco = new Player('Sylvian Francisco',[teamLogos[15],'BAY'],['G'], '185','26','1'); playersData.push(sylvianFrancisco)
    let carsenEdwards = new Player('Carsen Edwards',[teamLogos[15],'BAY','FEN'],['G'], '180','25','3'); playersData.push(carsenEdwards)
    let nielsGiffey = new Player('nielsGiffey',[teamLogos[15],'BAY','ALB','ZAL'],['F'], '200','32','7'); playersData.push(nielsGiffey)
    let dinoRadoncic = new Player('Dino Radoncic',[teamLogos[15],'BAY','RMB'],['F'], '203','25','8'); playersData.push(dinoRadoncic)
    let isaacBonga = new Player('Isaac Bonga',[teamLogos[15],'BAY'],['G'], '204','24','9'); playersData.push(isaacBonga)
    let leandroBolmaro = new Player('Leandro Bolmaro',[teamLogos[15],'BAY','FCB'],['G'], '198','23','10'); playersData.push(leandroBolmaro)
    let vladimirLucic = new Player('Vladimir Lucic',[teamLogos[15],'BAY','KKP','VAL'],['F'], '204','34','11'); playersData.push(vladimirLucic)
    let anderasObst = new Player('Andreas Obst',[teamLogos[15],'BAY'],['G'], '191','27','13'); playersData.push(anderasObst)
    let sergeIbaka = new Player('Serge Ibaka',[teamLogos[15],'BAY'],['F','C'], '208','34','14'); playersData.push(sergeIbaka)
    let jan_niklasWimberg = new Player('Jan Niklas Wimberg',[teamLogos[15],'BAY'],['F'], '206','28','17'); playersData.push(jan_niklasWimberg)
    let eliasHarris = new Player('Elias Harris',[teamLogos[15],'BAY'],['F'], '203','34','20'); playersData.push(eliasHarris)
    let dankoBrankovic = new Player('Danko Brankovic',[teamLogos[15],'BAY'],['C'], '216','23','22'); playersData.push(dankoBrankovic)
    let devinBooker = new Player('Devin Booker',[teamLogos[15],'BAY','FEN'],['F','C'], '205','32','31'); playersData.push(devinBooker)
    
    let mikeScott = new Player('Mike Scott',[teamLogos[16],'ASV'],['F'], '201','35','00'); playersData.push(mikeScott)
    let deshaunThomas = new Player('Deshaun Thomas',[teamLogos[16],'ASV','EA7','BAY','PAO','MAC','EFE','FCB'],['F'], '201','32','1'); playersData.push(deshaunThomas)
    let parisLee = new Player('Paris Lee',[teamLogos[16],'ASV','PAO','ASM'],['G'], '183','28','3'); playersData.push(parisLee)
    let charlesKahudi = new Player('Cahrles Kahudi',[teamLogos[16],'ASV'],['F'], '199','37','5'); playersData.push(charlesKahudi)
    let joffreyLauvergne = new Player('Jeffrey Lauvergne',[teamLogos[16],'ASV','VAL','KKP','FEN','ZAL'],['F','C'], '211','32','7'); playersData.push(joffreyLauvergne)
    let timotheLuwawu_Cabarrot = new Player('Timothe Luwawu-Cabarot',[teamLogos[16],'ASV','EA7'],['F'], '201','28','9'); playersData.push(timotheLuwawu_Cabarrot)
    let nandoDe_Colo = new Player('Nando de Colo',[teamLogos[16],'ASV'],['G'], '196','36','12'); playersData.push(nandoDe_Colo)
    let borisDallo = new Player('Boris Dallo',[teamLogos[16],'ASV','KKP'],['G','F'], '196','29','10'); playersData.push(borisDallo)
    let edwinJackson = new Player('Edwin Jackson',[teamLogos[16],'ASV','FCB'],['G'], '191','34','11'); playersData.push(edwinJackson)
    let youssouphaFall = new Player('Yousoupha Fall',[teamLogos[16],'ASV','BSK'],['C'], '221','29','19'); playersData.push(youssouphaFall)
    let davidLighty = new Player('David Lighty ',[teamLogos[16],'ASV'],['G','F'], '198','35','23'); playersData.push(davidLighty)
    let noamYaacov = new Player('Noam Yaacov',[teamLogos[16],'ASV'],['G'], '185','19','31'); playersData.push(noamYaacov)

    let sterlingBrown = new Player('Sterling Brown',[teamLogos[17],'ALB'],['G','F'], '196','29','0'); playersData.push(sterlingBrown)
    let gabrieleProcida = new Player('Gabirele Procida',[teamLogos[17],'ALB'],['G','F'], '198','21','1'); playersData.push(gabrieleProcida)
    let matteoSpagnolo = new Player('Matteo Spagnolo',[teamLogos[17],'ALB','RMB'],['G'], '193','21','3'); playersData.push(matteoSpagnolo)
    let yaniWetzell = new Player('Yani Wetzell',[teamLogos[17],'ALB','BSK'],['F','C'], '208','27','5'); playersData.push(yaniWetzell)
    let timSchneider = new Player('Tim Schneider',[teamLogos[17],'ALB'],['F'], '208','26','10'); playersData.push(timSchneider)
    let mattThomas = new Player('Matt Thomas',[teamLogos[17],'ALB','PAO'],['G'], '191','29','11'); playersData.push(mattThomas)
    let martinHermannsson = new Player('Martin Hermansson',[teamLogos[17],'ALB','VAL'],['G'], '190','29','15'); playersData.push(martinHermannsson)
    let zigaSamar = new Player('Ziga Samar',[teamLogos[17],'ALB'],['G'], '197','23','18'); playersData.push(zigaSamar)
    let louisOlinde = new Player('Louis Olinde',[teamLogos[17],'ALB'],['F'], '205','25','19'); playersData.push(louisOlinde)
    let christKoumadje = new Player('Christ Koumadje',[teamLogos[17],'ALB'],['C'], '224','27','21'); playersData.push(christKoumadje)
    let johannesThiemann = new Player('Johannes Thiemann',[teamLogos[17],'ALB'],['F','C'], '205','30','32'); playersData.push(johannesThiemann)
